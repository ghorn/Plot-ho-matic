...
data MultiCarousel = MultiCarousel{css :: !(P'.Seq Kite.CarouselState), messages :: !(P'.Seq P'.Utf8),
                                   currentState :: !(P'.Maybe Kite.CarouselState)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable MultiCarousel where
  mergeAppend (MultiCarousel x'1 x'2 x'3) (MultiCarousel y'1 y'2 y'3)
   = MultiCarousel (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default MultiCarousel where
  defaultValue = MultiCarousel P'.defaultValue P'.defaultValue P'.defaultValue
 
instance P'.Wire MultiCarousel where
  wireSize ft' self'@(MultiCarousel x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeRep 1 11 x'1 + P'.wireSizeRep 1 9 x'2 + P'.wireSizeOpt 1 11 x'3)
  wirePut ft' self'@(MultiCarousel x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             26 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{currentState = P'.mergeAppend (currentState old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
